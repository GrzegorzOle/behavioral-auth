...
def report():
    ...
    df = conn.execute('SELECT ts_utc, behavioral_score, howdy_score, fused_score, decision, mode FROM decisions ORDER BY ts_utc').fetchdf()
    if df.empty:
    ...
    if meta:
        ...
        print(json.dumps({'FAR_proxy': far, 'FRR_proxy': frr, 'EER_proxy': (far + frr) / 2.0}, indent=2))
